...
plot_raster_png <- function(path, palette_breaks) {
  ...
  # compute the breaks for a symmetrical colour scale
  # vel_min <-
  #   vel %>% summary() %>% pluck(1) %>% str_remove("Min.   :") %>% as.numeric()
  # vel_max <-
  #   vel %>% summary() %>% pluck(6) %>% str_remove("Max.   :") %>% as.numeric()
  # data_range <- c(vel_min, vel_max) %>% abs() %>% max()
  # number_colours <- 10
  # palette_breaks <-
  #   seq(-data_range, data_range, length.out = number_colours + 1)

  # make a symmetrical colour scale around 0 based on raster's range
...
city_maps_info %>%
  ...
  select(filename, ref_lon, ref_lat, file_min, file_max,
