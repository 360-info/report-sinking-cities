...
map_title = md`# Sinking cities: ${selectedCity}`
map_subtitle = md`Measurements from researchers found that parts of the city are sinking by as much as **${max_city_sink * 100} cm per year.**` 
```
...
legend =
  svg`<svg height="65px" width="${width * bins}px">
    <text fill="black" y=15 text-anchor="start" x=0>Sinking</text>
    ...
    <text fill="black" y=55 text-anchor="start" x=0>-${filteredCityInfo[0].palette_extent * 100} cm/yr</text>
    <text fill="black" y=55 text-anchor="end" x=${width * bins}>
      +${filteredCityInfo[0].palette_extent * 100} cm/yr
    </text>
  </svg>`
```
...
layer_0to25 = new lib.deck.MapboxLayer({
  ...
  // coordinateSystem: lib.deck.COORDINATE_SYSTEM.METER_OFFSETS
});
